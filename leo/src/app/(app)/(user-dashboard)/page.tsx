...
=======
  const { user } = getUser()
  const filters = {
    org: props.searchParams?.org || undefined,
  }

  // TODO - QUERY USING FILTERSs
  const { items: projects } = await db.query<ProjectRecordType>({
    table: 'Projects',
    index: {
  ...
  return (
    <main className="mx-auto max-w-[1260px]">
      {/* <div className="mb-12 flex flex-row gap-10">
    <main className="mx-auto mt-16 max-w-[1000px]">
      <div className="mb-12 flex flex-row gap-10">
>>>>>>> b030934 (progress)
        <input
...
=======
        <Link
          href="/~/create"
          className="rounded-md border border-neutral-800 bg-neutral-900 px-6 py-2 text-neutral-300 hover:bg-neutral-800"
        >
          Create Project
        </Link>
      </div>
