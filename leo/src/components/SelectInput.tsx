      ...
      <Combobox value={SELECTED_OPTION} onChange={SET_SELECTED_OPTION}>
        ...
        <div className="relative w-full bg-inherit text-sm">
          <Combobox.Input
            ...
            name={props.name}
            autoComplete={'off'}
            className={`w-full rounded-md border border-neutral-700 bg-neutral-900 px-2 py-1.5 text-neutral-200 invalid:border-red-400 invalid:bg-red-50 ${props.inputClass}`}
        ...
        {/* Options: */}
        <Combobox.Options className="absolute z-50 mt-1 max-h-60 w-full overflow-auto rounded-md bg-neutral-800 py-1 text-base shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none sm:text-sm">
          {FilteredOptions.length === 0 && query !== '' ? (
            ...
            FilteredOptions.map((option) => (
              <Combobox.Option
                ...
                value={option}
                className={({ active }) =>
                  `relative cursor-default select-none px-2 py-2 pr-4 ${active ? 'bg-blue-500 text-white' : ''}`
                }
              >
